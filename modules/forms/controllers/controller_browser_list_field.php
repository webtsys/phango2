		...
		if($yes_go==0)
		{
			...
			if(isset(PhangoVar::$model[$model_name]) && isset(PhangoVar::$model[$model_name]->components[$field_ident]))
			{
				...
				if($category_model!='' && isset(PhangoVar::$model[$category_model]) && isset(PhangoVar::$model[$category_model]->components[$category_model_field]) && isset(PhangoVar::$model[$model_name]->components[$category_field]) )
				{
					...
					{
						echo load_view(array('title' => PhangoVar::$l_['common']->lang('filter_by_category', 'Filter by category'), SelectModelFormByOrder('category_id', '', $_GET['category_id'], $model_name, $category_model, $field_parent_category, $where='', $null_yes=1) ), 'content');
					}
					else
					{*/
						
					
					$form_html='<form method="get" action="'.$url_options.'/">'.SelectModelForm('category_id', '', $_GET['category_id'], $category_model, $category_model_field, '').'<input type="submit" value="'.PhangoVar::$l_['common']->lang('send', 'Send').'"/></form>';
					
					echo load_view(array('title' => PhangoVar::$l_['common']->lang('filter_by_category', 'Filter by category'),  $form_html), 'content');
			...
			$title=''; //PhangoVar::$l_['forms']->lang('search_on_table', 'Search on table');
...
function ChooseOptionsListModel($url_options, $model_name, $id, $arr_row)
{
	...
	$arr_options[]='<a href="#" class="select_id select_id_'.$id.'"><span style="display:none;" id="text_field_'.$id.'">'.$arr_row[$field_ident].'</span>'.PhangoVar::$l_['common']->lang('select', 'select').'</a>';
