...
function generate_edit_model_method_class($class, $idrow, $arr_fields, $url_admin, $url_back, $yes_insert=1, $where_sql='')
{
	...
	$label=common_l('Add new element').' - '.$class->label;
	...
	if($class->element_exists($idrow))
	{
		...
		$label=common_l('Edit').' - '.$class->label;
	...
	if($yes_insert==0)
	{
		...
		echo common_l('Cannot insert or update this item in the database');
	...
	switch($_GET['op_update'])
	{
		...
		?>
		<p><a href="<?php echo $url_back; ?>"><?php echo common_l('Go back'); ?></a></p>
		<?php
		...
		case 1:
			...
			else
			{
				...
				echo '<p class="error">'.common_l('Cannot insert or update this item in the database').' '.$class->name.': '.$class->std_error.'</p>';
