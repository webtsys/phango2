...
class BooleanField extends PhangoField {
	...
	*/
	...
	function get_parameters_default()
	{
		...
		$arr_values=array($this->default_value, common_l('No'), 0, common_l('Yes'), 1);;
...
class FileField extends PhangoField {
	...
	{	
		...
		if(isset($_FILES[$file_field]['tmp_name']))
		{
			...
			if($_FILES[$file_field]['tmp_name']!='')
			{
				...
				else
				{
					...
					$this->std_error=common_l('Error: Cannot upload this file to the server');
		...
		else
		{
			...
			$this->std_error=error_model_l('Please, check enctype form of file form');
	...
	function process_delete_field($model, $name_field, $conditions)
	{
		...
		while(list($file_name)=webtsys_fetch_row($query))
		{
			...
			if(!unlink($this->path.'/'.$file_name))
			{
				...
				$this->std_error=common_l('Cannot delete the file');
			...
			{	
				...
				if($this->min_size[0]>0 && $this->min_size[1]>0)
				{
					...
					if($arr_image[0]<$this->min_size[0] || $arr_image[1]<$this->min_size[1])
					{
						...
						$this->std_error=common_l('Image size is wrong').'<br />'.common_l('Minimal size').': '.$this->min_size[0].'x'.$this->min_size[1];
				...
				if(file_exists($this->path.'/'.$_FILES[$file]['name']))
				{
					...
					$this->std_error=common_l('There is an image with the same name');
				...
				if($arr_image[2]==1 || $arr_image[2]==2 || $arr_image[2]==3)
				{
					...
					else
					{
						...
						$this->std_error=common_l('Error: Cannot upload this image to the server');
				...
				else
				{
					...
					$this->std_error.=error_model_l('Img format error, only accept gif, jpg and png formats');
				...
				if(unlink($this->path.'/'.$image_name))
				{
					...
					foreach($this->img_width as $key => $value)
					{
						...
						if(!unlink($this->path.'/'.$key.'_'.$image_name))
						{
							...
							$this->std_error.=common_l('Cannot delete the image').': '.$key.'_'.$image_name;
						
						}
					
					}
				
					$this->std_error.=common_l('Cannot delete the image').': '.$image_name;
	...
	*/
	...
	function get_parameters_default()
	{
		...
		else
		{
			...
			$arr_values=array('', common_l('Any option chosen'), '');
...
class EmailField extends PhangoField {
	...
	function check($value)
	{
		...
		else
		{
			...
			$this->std_error.=error_model_l('Email format error').' ';
